      ...
      <div style={{ maxWidth: 480, margin: "0 auto", paddingBottom: 40 }}>
        ...
        <div style={{ padding: "24px 20px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
            {seriesDetail.genre && <span style={{ fontSize: 11, background: "rgba(45,31,78,0.9)", color: "#a78bfa", borderRadius: 20, padding: "3px 10px" }}>{seriesDetail.genre}</span>}
            ...
            {seriesDetail.tags && seriesDetail.tags.split(",").slice(0, 3).map((t) => (
              <span key={t} style={{ fontSize: 11, background: "rgba(28,21,48,0.9)", color: "#7a6a9a", borderRadius: 20, padding: "3px 10px", border: "1px solid #2e2048" }}>#{t.trim()}</span>
            ))}
          </div>
          ...
          {!isMine && (
            <button
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", background: seriesDetail.is_favorited ? "rgba(45,31,78,0.9)" : "transparent", border: `1.5px solid ${seriesDetail.is_favorited ? "#7c3aed" : "#2e2048"}`, borderRadius: 24, color: seriesDetail.is_favorited ? "#c4b8ff" : "#7a6a9a", cursor: "pointer", fontFamily: "'Noto Serif KR', serif", fontSize: 14, marginBottom: 20, transition: "all 0.2s" }}
          ...
          {isMine && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                <label style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "11px", background: "transparent", border: "1.5px solid #2e2048", borderRadius: 12, color: "#a89ec0", cursor: "pointer", fontSize: 13, fontFamily: "'Noto Serif KR', serif" }}>
              ...
              {/* 전체 공개 토글 */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "rgba(19,16,32,0.85)", borderRadius: 10, border: "1px solid #2e2048", marginBottom: 8 }}>
                <span style={{ fontSize: 13, color: "#b8aed0" }}>전체 공개 설정</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 12, color: allPublic ? "#7c3aed" : "#6a5a8a" }}>{allPublic ? "🌍 공개" : "🔒 비공개"}</span>
                  <label style={{ position: "relative", width: 44, height: 24, cursor: "pointer" }} onClick={() => onToggleSeriesPublic(sid, seriesDetail.id, !allPublic)}>
                    <span style={{ position: "absolute", inset: 0, background: allPublic ? "#7c3aed" : "#2e2048", borderRadius: 24, transition: "0.3s" }}>
          ...
          {/* 목차 */}
          <div
            style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "rgba(19,16,32,0.85)", borderRadius: showToc ? "12px 12px 0 0" : 12, border: "1px solid #2e2048", cursor: "pointer" }}
            onClick={() => setShowToc(!showToc)}
          >
            <span style={{ fontSize: 13, fontWeight: 600, color: "#b8aed0" }}>
              📋 목차 <span style={{ color: "#7c3aed", marginLeft: 4 }}>{eps.length}화</span>
            </span>
            <span style={{ fontSize: 13, color: "#6a5a8a", display: "inline-block", transform: showToc ? "rotate(180deg)" : "rotate(0deg)", transition: "0.2s" }}>▾</span>
          </div>
          {showToc && (
            <div style={{ border: "1px solid #2e2048", borderTop: "none", borderRadius: "0 0 12px 12px", overflow: "hidden" }}>
              {eps.map((ep, idx) => (
                <div
                  key={ep.id}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "rgba(19,16,32,0.85)", borderTop: idx > 0 ? "1px solid #2e2048" : "none", cursor: "pointer" }}
                  onMouseOver={(e) => (e.currentTarget.style.background = "rgba(28,21,48,0.9)")}
                  onMouseOut={(e) => (e.currentTarget.style.background = "rgba(19,16,32,0.85)")}
                  onClick={() => onOpenEpisode(ep, isMine)}
                >
                  <div style={{ width: 32, height: 32, borderRadius: 6, background: "rgba(45,31,78,0.9)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#a78bfa", fontWeight: 600 }}>{ep.episode_number}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#ede8f5", marginBottom: 2 }}>{ep.episode_number}화. {ep.title}</div>
                    <div style={{ fontSize: 11, color: "#6a5a8a" }}>{new Date(ep.created_at).toLocaleDateString("ko-KR")}</div>
                  </div>
                  {isMine && (
                    <button
                      style={{ background: "transparent", border: "1px solid #2e2048", borderRadius: 8, padding: "4px 10px", color: "#7a6a9a", cursor: "pointer", fontSize: 11, fontFamily: "'Noto Serif KR', serif", flexShrink: 0 }}
