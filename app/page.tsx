...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const saveSynopsis = generatedSynopsis || null; // AI 생성 소개만 저장
...
- 이름 없는 캐릭터는 어울리는 이름을 직접 정해서 쓸 것${ratingGuide}`;
  ...
  async function generateNovel() {
    ...
    const charDesc = characters.filter(c => c.name || c.desc || c.age || c.gender || c.role).map(c => {
      ...
      return `- [${c.role}]${ageGender ? ` (${ageGender})` : ""}${rel} 이름: ${c.name || "AI가 정할 것"} / 특징: ${c.desc || "없음"}`;
    }).join("\n");
...
3. 장면 구성:
   ...
   - 장면3: [감정 클라이맥스 장면]
4. 캐릭터 말투 설계:
   - 각 주요 캐릭터의 말투 특징 (예: 건조하고 짧게 말함, 존댓말 유지, 빈정대는 어투 등)
   - 두 인물 사이 대화 톤 (긴장감/부드러움/적대감 등)
5. 감정 흐름: 시작 → 전환점 → 끝
